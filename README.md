# _AlphaSyndrome_: Tackling the Syndrome Measurement Circuit Scheduling Problem for QEC Codes

This repo contains the code for paper _AlphaSyndrome: Tackling the Syndrome Measurement Circuit Scheduling Problem for QEC Codes_.

## Setup

We recommend using virtual environment with Python to initialize the packages. After set up the virtual environment, install packages with the provided `requirements.txt`.

## Evaluation

Using `evaluate-*.py` script to reproduce evaluation and scheduling. For more information, follow the command line interface and artifact evaluation guide from the original paper.

## Citation

```bibtex
@inproceedings{alphasyndrome,
...
pages = {77–93},
numpages = {17},
keywords = {quantum computing, quantum error correction, syndrome measurement},
location = {USA},
series = {ASPLOS '26}
}
```
